<x-admin-layout>
    <div class="space-y-6">
        <!-- Header Section -->
        <div class="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl shadow-xl">
            <div class="px-8 py-8 text-white">
                <div class="flex items-center justify-between">
                    ...
                    @can('create users')
                    <a href="{{ route('admin.users.create') }}" class="bg-white/20 hover:bg-white/30 text-white font-semibold py-3 px-6 rounded-xl transition-all duration-300 transform hover:scale-105 flex items-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                <div class="flex items-center">
                    ...
                    <div>
                        <p class="text-sm font-medium text-gray-600">Roles Assigned</p>
                        <p class="text-2xl font-bold text-gray-900">{{ $users->where('roles_count', '>', 0)->count() }}</p>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="bg-gray-50 rounded-xl p-6 hover:bg-gray-100 transition-all duration-300 transform hover:scale-105 border border-gray-200">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center space-x-2 pt-3 border-t border-gray-200">
                                @can('edit users')
                                <a href="{{ route('admin.users.edit', $user) }}" class="flex-1 bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium py-2 px-3 rounded-lg transition-colors duration-200 text-center">
                                    Edit
                                </a>
                                @endcan
                                @can('delete users')
                                <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="flex-1">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-full bg-red-500 hover:bg-red-600 text-white text-sm font-medium py-2 px-3 rounded-lg transition-colors duration-200" onclick="return confirm('Are you sure you want to delete this user?')">
                                        Delete
                                    </button>
                                </form>
                                @endcan
