                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('api-keys.store') }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="name" id="name" value="{{ old('name') }}" required
                                           placeholder="e.g., KuCoin Main Account"
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="password" name="api_key" id="api_key" value="{{ old('api_key') }}" required
                                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                    @error('api_key')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div id="passphrase_field" style="display: none;">
                        ...
                        <div class="mt-8 space-y-4">
                            ...
                            <p class="text-sm text-gray-600">Select the permissions this API key should have. Trading bots require "trade" permission.</p>
                            
                            <div class="space-y-3">
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="read" {{ in_array('read', old('permissions', ['read'])) ? 'checked' : '' }}
                                           class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    <span class="ml-2 text-sm text-gray-700">Read (View account information, balances, orders)</span>
                                </label>
                                
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="trade" {{ in_array('trade', old('permissions', ['trade'])) ? 'checked' : '' }}
                                           class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="transfer" {{ in_array('transfer', old('permissions', [])) ? 'checked' : '' }}
                                           class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
